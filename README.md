...
- **Login Seguro e Sessão Global:** Toda a sua conversa e o dashboard estão isolados e atrelados ao seu CPF em um banco de dados Supabase na nuvem.
...
2. **Servidor API (`api_server.py`):** Desenvolvido em FastAPI rodando na porta **8080**, que hospeda os arquivos estáticos, as rotas de autenticação (Login/Registro) e entrega os JSONs financeiros que abastecem o Dashboard em tempo real.
3. **Chat Engine (`chat_app.py`):** Construído em Chainlit, processa o áudio, lida com a interceptação das *Tools* para controlar o UI do dashboard, e gera as mensagens na tela usando *Streaming*.
4. **Servidor de Ferramentas (`mcp_server.py`):** Expõe as funções matemáticas e de banco de dados no padrão Model Context Protocol.
5. **Cérebro (`agent.py`):** O Grafo de Estado (LangGraph) do agente, conectando o LLM (OpenAI) ao MCP.

## 🚀 Como Executar o Projeto Localmente

A aplicação é **100% conteinerizada**, pronta para produção.

### 1. Pré-requisitos
- Docker Desktop instalado.
- Chave da API da **OpenAI** (`OPENAI_API_KEY`).
- Um projeto no **Supabase** configurado com as tabelas do projeto (veja a pasta `sql/`).
